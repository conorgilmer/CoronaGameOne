...
--Sounds
local coinSound, overSound, winSound, jumpSound, laserSound
local coinChannel, jumpChannel, overChannel, winChannel, laserChannel = 2,3,4,5, 7 --Channel vars, used to play sounds, these values change when they are used.
...
--	jumpSound = audio.loadSound("sounds/HellYeah.ogg")
        ...
        jumpSound = audio.loadSound("sounds/cartoon-jump.wav")
        laserSound = audio.loadSound("sounds/spaceblaster.wav")
		...
		if movementAllowed then
                 ...
                 physics.addBody( bullet,  "dynamic", { friction=1, bounce=0, shape=bulletShape} )
                   -- physics.addBody( bullet, "dynamic", { isSensor = true } )
                    ...
                    print("before call col")
                    Runtime:addEventListener("collision", bullet)
                    print ("after call col")
	...
	local jumpButton = display.newImageRect(extraGroup, "images/button.png", 54, 54)
	...
	downButton:addEventListener("touch", moveButton)
        ...
        local kickButton = display.newImageRect(extraGroup, "images/button.png", 54, 54)
...
local function createBullet(x,y)
		--Creation needs to be delayed slightly so the world isn't "Number crunching"
		local function createNow()
			local bullet = display.newImageRect(objectGroup, "images/bullet.png", 30, 30)
			bullet:setReferencePoint(display.BottomCenterReferencePoint);
			bullet.x = x; bullet.y = y;
                       -- -60; bullet.name = "bullet"
			physics.addBody( bullet, "dynamic", { isSensor = true } )
		end
		timer.performWithDelay(10, createNow, 1)
	end
		...
		if event.phase == "began" and gameIsActive == true and gameOverCalled == false then
			...
			local name2 = event.object2.name 
                        ...
                        if self.name =="bullet" then
                            ...
                            print "in if"
                        if name1 == "enemy" or name1 == "enemy" then
